...
{
    api->getAuth().m_access_token = QString{};
    api->getAuth().m_refresh_token = QString{};
...
void ImgurWindow::apiRequestPin(const QUrl &url)
{
    ...
    QString pin =
            QInputDialog::getText(this,
                                  i18n("Imgur API Authorization"),
                                  i18n("Open<br/>"
                                       "<a href='%1'>%1</a><br/>"
                                       "in a browser (if not done automatically)<br/>"
                                       "and copy the PIN here:")
...
void ImgurWindow::readSettings()
{
    ...
    api->getAuth().m_access_token = groupAuth.readEntry("access_token", QString());
    api->getAuth().m_refresh_token = groupAuth.readEntry("refresh_token", QString());
...
void ImgurWindow::saveSettings()
{
    KConfig config(QString::fromLatin1("kipirc"));
    KConfigGroup groupAuth = config.group("Imgur Auth");
    groupAuth.writeEntry("access_token", api->getAuth().m_access_token);
    groupAuth.writeEntry("refresh_token", api->getAuth().m_refresh_token);
